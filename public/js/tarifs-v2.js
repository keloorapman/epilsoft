...
document.addEventListener('DOMContentLoaded', function () {
    ...
    (function() {
        ...
        function makeBtns(container, items, selected, onSelect){
            if (!container) return;
            ...
            items.forEach(item => {
                ...
                b.textContent = item;
                b.className = 'sgz-btn';
                if(item === selected) b.classList.add('selected');
                b.onclick = () => { onSelect(item); };
                container.appendChild(b);
            });
        }
        function makeZones(){
            if (!zoneEl || !individualZonesData[selGender]) return;
            // Use safe DOM methods instead of innerHTML
            while (zoneEl.firstChild) {
                zoneEl.removeChild(zoneEl.firstChild);
            }
            const currentGenderZones = Object.keys(individualZonesData[selGender]);
            if (!selZone || !currentGenderZones.includes(selZone)) {
                selZone = currentGenderZones[0];
            }
            currentGenderZones.forEach(z => {
                const b = document.createElement('button');
                b.textContent = z;
                b.className = 'sgz-btn sgz-zone-btn';
                if(z === selZone) b.classList.add('selected');
                b.onclick = () => { selZone = z; update(); };
                zoneEl.appendChild(b);
            });
        }
        function makeTable(){
            let isForfaitMode = selOption === 'Forfaits', catData, tableTitle = 'Zones du corps';
            if (isForfaitMode) {
                catData = forfaitsData[selGender] || {};
                tableTitle = 'Forfaits Disponibles';
            } else {
                catData = individualZonesData[selGender]?.[selZone] || {};
            }
            if (!tblEl) return;
            ...
            thZone.textContent = tableTitle;
            headRow.appendChild(thZone);
            const priceHeaderCell = document.createElement('th');
            headRow.appendChild(priceHeaderCell);
            const tBody = tblEl.createTBody();

            Object.entries(catData).forEach(([zoneName, vals]) => {
                if (zoneName.startsWith('__header_')) {
                    const title = zoneName.replace('__header_', '').replace(/_/g, ' ');
                    const headerRow = tBody.insertRow();
                    headerRow.className = 'sgz-subheader';
                ...
                if (isForfaitMode) {
                    priceHeaderCell.textContent = 'Prix à la séance';
                    const span = document.createElement('span');
                    span.className = 'price-small';
                    span.textContent = (vals.unit !== undefined ? vals.unit : 'N/A') + ' €';
                    priceCell.appendChild(span);
                } else {
                    if (selOption === sessionsAndForfait[0]) {
                        priceHeaderCell.textContent = 'Prix à la séance';
                        const span = document.createElement('span');
                        span.className = 'price-small';
                        span.textContent = (vals.unit !== undefined ? vals.unit : 'N/A') + ' €';
                        priceCell.appendChild(span);
                    } else if (selOption === sessionsAndForfait[1]) {
                        priceHeaderCell.textContent = 'Pack 5 séances (-25%)';
                        const perSessionText5 = vals.per5 !== undefined ? vals.per5.toFixed(2).replace('.',',') : 'N/A';

                        const strikeSpan = document.createElement('span');
                        strikeSpan.className = 'strike';
                        strikeSpan.textContent = (vals.total5 !== undefined ? vals.total5 : 'N/A') + ' €';
                        priceCell.appendChild(strikeSpan);
                        priceCell.appendChild(document.createTextNode(' '));

                        const priceSpan = document.createElement('span');
                        priceSpan.className = 'price-large';
                        priceSpan.textContent = (vals.total5disc !== undefined ? vals.total5disc.toFixed(2).replace('.', ',') : 'N/A') + ' €';
                        priceCell.appendChild(priceSpan);

                        const small = document.createElement('small');
                        small.textContent = '(' + perSessionText5 + ' €/séance)';
                        priceCell.appendChild(small);
                    } else if (selOption === sessionsAndForfait[2]) {
                        priceHeaderCell.textContent = 'Pack 8 séances (-35%)';
                        const perSessionText8 = vals.per8 !== undefined ? vals.per8.toFixed(2).replace('.',',') : 'N/A';

                        const strikeSpan = document.createElement('span');
                        strikeSpan.className = 'strike';
                        strikeSpan.textContent = (vals.total8 !== undefined ? vals.total8 : 'N/A') + ' €';
                        priceCell.appendChild(strikeSpan);
                        priceCell.appendChild(document.createTextNode(' '));

                        const priceSpan = document.createElement('span');
                        priceSpan.className = 'price-large';
                        priceSpan.textContent = (vals.total8disc !== undefined ? vals.total8disc.toFixed(2).replace('.', ',') : 'N/A') + ' €';
                        priceCell.appendChild(priceSpan);

                        const small = document.createElement('small');
                        small.textContent = '(' + perSessionText8 + ' €/séance)';
                        priceCell.appendChild(small);
                    }
                }
            });
